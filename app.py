...
def home():
    if request.method == 'POST':
        try:
            ...
            if img is None: return "Error: Image decode failed", 400
            ...
            bh, bw = bordered.shape[:2] # Boundary dynamic le li (433, 551)
            ...
            for i in range(min(count, 12)):
                r, c = i // 3, i % 3
                # Yahan calculation dynamic kar di taaki shape mismatch na ho
                y_start = r * (bh + 40) + 50
                x_start = c * (bw + 20) + 50
            ...
            if filetype == "jpg":
                ...
                return send_file(io.BytesIO(encoded_img.tobytes()), mimetype='image/jpeg', as_attachment=True, download_name='passport.jpg')
            else:
                pdf_io = io.BytesIO()
                c_pdf = pdf_canvas.Canvas(pdf_io, pagesize=A4)
                _, encoded_img = cv2.imencode('.jpg', canvas_img)
                img_reader = ImageReader(io.BytesIO(encoded_img.tobytes()))
                c_pdf.drawImage(img_reader, 50, 100, width=500, height=650)
                c_pdf.showPage()
                c_pdf.save()
                pdf_io.seek(0)
